...
export async function generateMetadata({
  ...
  const procedure = await createProcedureAdapter().getBySlug(safeLocale, slug);
  if (!procedure) {
    return {};
  }
  ...
  return {
    title: `${procedure.title} | ${SITE_NAME}`,
...
export default async function ProcedureDetailPage({
  ...
  const detail = messages.pages.procedure_detail;
  ...
  return (
    <div className="space-y-6">
      <nav className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
        ...
        <Link href={getLocalePath(safeLocale, "procedures")} className="hover:text-brand-dark dark:hover:text-brand-light">
          {messages.header.navigation.procedures}
        </Link>
        <span aria-hidden="true">/</span>
        <span className="font-medium text-slate-900 dark:text-slate-100">{procedure.title}</span>
      </nav>
      ...
      <section className="section-card p-7 md:p-10">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          >
            {messages.common.back_to_procedures}
          </Link>
        </div>
      </section>
      ...
      <section className="section-card p-7 md:p-9">
        ...
        {procedure.beforeAfterImages.length > 0 ? (
